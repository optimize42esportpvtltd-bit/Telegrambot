...
# Configuration
# ==========================================
BOT_TOKEN = "8871377583:AAEVP762W0dY7Tm8dmp0SeXZah-DISKkup0"
ADMIN_ID = 1841031375  # আপনার Telegram User ID দিন
BASE_URL = f"https://api.telegram.org/bot{BOT_TOKEN}/"
BOT_USERNAME = "smsifyhub_bot"

# 2oo9 Live Access API Configuration
LIVE_ACCESS_URL = "https://api.2oo9.cloud/MXS47FLFX0U/tnevs/@public/api/liveaccess"
LIVE_ACCESS_KEY = "M7IDA8HXCCQ"

# SMSIFYHUB API Configuration
SMSIFY_BASE = "https://panel.smsifyhub.site/api"
SMSIFY_KEY_DEFAULT = "smsify_dbd5f73cab7b3eac26b10943cd53d818"
...
# User & Session States
user_cache = {} 
active_sessions = {} # {session_id: {"number": num, "user_id": uid, "service": sid, "start_time": t}}
user_active_sessions = {} 
user_cooldowns = {} 

# Auto System States
smsify_auto_mode = True 
smsify_keys = [SMSIFY_KEY_DEFAULT]
force_join_status = False
force_join_channels = []

# OTP Forwarding States
otp_forward_groups = []
otp_button_link = "https://t.me/smartotpv1bot"
...
# ==========================================
# Admin Settings (BABY Control)
# ==========================================
bot_settings = {
    "withdraw_on": True,
    "min_withdraw": 10.0,
    "support_link": "https://t.me/smartotpv1bot",
    "tg_group_link": "https://t.me/smartotpv1bot",
    "w_group": "",
    "auto_br_on": False,
    "auto_br_interval": 60,
    "cooldown": 5, 
...
# ==========================================
def get_api_key():
    return random.choice(smsify_keys) if smsify_keys else SMSIFY_KEY_DEFAULT
...
# ==========================================
def load_local_data():
    global recent_traffic, smsify_dynamic_data, smsify_keys, force_join_status, force_join_channels, smsify_auto_mode, otp_forward_groups, otp_button_link, recent_success_otps, bot_settings, user_active_sessions, daily_stats, daily_user_otps, active_sessions
    if os.path.exists(BOT_DATA_FILE):
        try:
            with open(BOT_DATA_FILE, "r") as f:
                data = json.load(f)
                recent_traffic = data.get("recent_traffic", [])
                smsify_dynamic_data = data.get("smsify_dynamic_data", {})
                loaded_keys = data.get("smsify_keys", [])
                
                if not loaded_keys:
                    smsify_keys = [SMSIFY_KEY_DEFAULT]
                else:
                    smsify_keys = loaded_keys
                    
                force_join_status = data.get("force_join_status", False)
                force_join_channels = data.get("force_join_channels", [])
                smsify_auto_mode = data.get("smsify_auto_mode", True)
                otp_forward_groups = data.get("otp_forward_groups", [])
                otp_button_link = data.get("otp_button_link", "https://t.me/smartotpv1bot")
...
def save_local_data():
    global recent_traffic, smsify_dynamic_data, smsify_keys, force_join_status, force_join_channels, smsify_auto_mode, otp_forward_groups, otp_button_link, recent_success_otps, bot_settings, user_active_sessions, daily_stats, active_sessions
    try:
        with open(BOT_DATA_FILE, "w") as f:
            json.dump({
                "recent_traffic": recent_traffic, 
                "smsify_dynamic_data": smsify_dynamic_data,
                "smsify_keys": smsify_keys,
                "force_join_status": force_join_status,
                "force_join_channels": force_join_channels,
                "smsify_auto_mode": smsify_auto_mode,
...
def get_all_user_ids():
    ...
    if os.path.exists(USERS_LIST_FILE):
        try:
            with open(USERS_LIST_FILE, "r") as f:
                for u in json.load(f): users.add(u)
        except Exception: pass
    try:
        conn = sqlite3.connect("bot_database.db")
        cursor = conn.cursor()
        cursor.execute("SELECT user_id FROM users")
        for row in cursor.fetchall(): users.add(row[0])
        conn.close()
    except Exception: pass
    if current_db_mode == "firebase" and db_firebase:
        try:
            docs = db_firebase.collection('users').stream()
            for doc in docs: users.add(int(doc.id))
        except Exception: pass
    return list(users)
...
# ==========================================
# Global Body Emojis Map
# ==========================================
GLOBAL_BODY_EMOJIS = {
    "➖": "5870818207383686839", "🚫": "5334807341109908955", "😒": "5334763399299506604",
    "🖥": "5172893417717367746", "🌐": "5334590977837403844", "🌟": "5337102391244263212",
...
# ==========================================
# 242 Total Countries Flag & Emoji IDs
# ==========================================
COUNTRY_EMOJI_IDS = {
    ...
    "🇲🇴": "6323557758096377611", "🇦🇼": "5292166459118606932", "🇨🇼": "5292166459118606932",
    "🇫🇰": "5913443365499703513", "🇬🇫": "5913605586414473124", "🇬🇵": "5913605586414473124",
    "🇷🇪": "5913605586414473124", "🇳🇨": "5913605586414473124", "🇳🇺": "5913640044937089340",
    "🇳🇫": "5913632326880858455", "🇼🇫": "5913605586414473124", "🇨🇰": "5913640044937089340",
    "🇵🇫": "5913605586414473124", "🇬🇮": "5913443365499703513", "🇵🇲": "5913605586414473124"
}
...
def apply_emojis(text):
    ...
    for char, eid in COUNTRY_EMOJI_IDS.items():
        text = text.replace(char, f'<tg-emoji emoji-id="{eid}">{char}</tg-emoji>')
...
def get_country_info(range_str):
    clean_range = str(range_str or "").replace("X", "").replace("x", "")
    for code, name in sorted(ALL_COUNTRY_CODES.items(), key=lambda x: len(x[0]), reverse=True):
...
def get_service_info_html(service_name):
    app_info = PREMIUM_APPS.get(service_name, PREMIUM_APPS.get(service_name.capitalize(), PREMIUM_APPS["Other"]))
...
def generate_otp_display(service_name, raw_number, message_text):
    ...
    clean_num = str(raw_number).lstrip('+')
    if len(clean_num) > 6:
        masked_number = f"{clean_num[:3]}BABY{clean_num[-3:]}"
    else:
        masked_number = clean_num
    ...
    clean_range = clean_num[:5] if len(clean_num) >= 5 else clean_num
...
# ==========================================
# 2oo9 API & SMSIFYHUB Methods
# ==========================================
def fetch_live_access():
    """
    2oo9 Live Access API থেকে লাইভ সার্ভিস এবং তার ভেতরের রেঞ্জসমূহ টেনে আনা
    """
    headers = {
        "mauthapi": LIVE_ACCESS_KEY,
        "X-Api-Key": LIVE_ACCESS_KEY,
        "Authorization": f"Bearer {LIVE_ACCESS_KEY}",
        "User-Agent": "Mozilla/5.0"
    }
    params = {"api_key": LIVE_ACCESS_KEY}
    try:
        res = requests.get(LIVE_ACCESS_URL, headers=headers, params=params, timeout=8)
        data = res.json()
        if data.get("meta", {}).get("code") == 200:
            ...
            print(f"[2oo9 API Debug] Response: {res.text}")
    except Exception as e:
        print(f"[2oo9 API Error] {e}")
    return []

def smsify_get_number(target_range, service="any"):
    """
    SMSIFYHUB API থেকে নম্বর নেওয়া
    """
    api_key = get_api_key()
    url = f"{SMSIFY_BASE}/get-number.php"
    params = {
        "api_key": api_key,
        "range": target_range,
        "service": service
    ...
    try:
        ...
        data = res.json()
        if data.get("status") in ["success", True, 200] or "number" in data:
            return data
    except Exception:
        pass
    return None

def smsify_get_otp(session_id):
    """
    SMSIFYHUB API থেকে ওটিপি চেক করা
    """
    api_key = get_api_key()
    url = f"{SMSIFY_BASE}/get-otp.php"
    params = {
        "api_key": api_key,
...
# ==========================================
def live_access_feed_listener():
    """
    2oo9 API থেকে লাইভ সার্ভিস এবং ট্রাফিক আপডেট করা
    """
    global smsify_dynamic_data, recent_traffic
    while True:
        try:
            live_services = fetch_live_access()
            if live_services:
                current_time = int(time.time())
                new_traffic_items = []
                
                for srv in live_services:
                    sid = srv.get("sid", "Other")
                    ranges = srv.get("ranges", [])
                    last_at = srv.get("last_at", current_time)
                    
                    app_info = PREMIUM_APPS.get(sid, PREMIUM_APPS.get(sid.capitalize(), PREMIUM_APPS["Other"]))
                    emoji_id, normal_emoji = app_info["id"], app_info["emoji"]
                    
                    if sid not in smsify_dynamic_data:
                        smsify_dynamic_data[sid] = {"emoji_id": emoji_id, "normal_emoji": normal_emoji, "ranges": {}}
                        
                    for r in ranges:
                        clean_r = str(r).replace("X", "").replace("x", "")
                        if clean_r:
                            if clean_r not in smsify_dynamic_data[sid]["ranges"]:
                                smsify_dynamic_data[sid]["ranges"][clean_r] = {"last_seen": current_time, "hits": 0}
                            smsify_dynamic_data[sid]["ranges"][clean_r]["last_seen"] = current_time
                            smsify_dynamic_data[sid]["ranges"][clean_r]["hits"] += 1
                            
                            new_traffic_items.append({
                                "service": sid,
                                "range": clean_r,
                                "time": last_at,
                ...
                if new_traffic_items:
                    recent_traffic = (new_traffic_items + recent_traffic)[:60]
                save_local_data()
        except Exception as e:
            print(f"[Live Access Listener Error]: {e}")
        time.sleep(6)

def otp_polling_listener():
    """
    ১ সেকেন্ড পর পর SMSIFYHUB API থেকে ইনস্ট্যান্ট ওটিপি চেক করে ডেলিভারি
    """
    global active_sessions, recent_success_otps, otp_forward_groups, daily_user_otps
    group_sent_otps = set()

    while True:
        try:
            sessions_copy = list(active_sessions.items())
            for session_id, s_info in sessions_copy:
                now = time.time()
                if now - s_info.get("start_time", now) > 900:
                    del active_sessions[session_id]
                    continue

                res = smsify_get_otp(session_id)
                if not res: continue

                status = res.get("status", "")
                otp_data = res.get("data", res)
                
                sms_message = ""
                if isinstance(otp_data, dict):
                    sms_message = otp_data.get("message", otp_data.get("sms", otp_data.get("otp", "")))
                elif isinstance(res, dict):
                    sms_message = res.get("message", res.get("sms", res.get("otp", "")))

                if (status in ["success", 200, True] or sms_message) and sms_message:
                    raw_number = s_info.get("number", "")
                    user_id = s_info.get("user_id")
                    service = s_info.get("service", "Unknown")

                    sig = f"{raw_number}_{sms_message}"
                    detected_service = detect_service(sms_message, service)

                    if sig not in group_sent_otps:
                        group_sent_otps.add(sig)
                        if otp_forward_groups:
                            formatted_message, markup = generate_otp_display(detected_service, raw_number, sms_message)
                    ...
                    if sig not in recent_success_otps:
                        ...
                        update_user_stats(user_id, reward, 0)

                        user_balance = get_user(user_id).get("balance", 0.0)
                        deliver_to_inbox(user_id, detected_service, raw_number, sms_message, user_balance, reward)
...
def auto_cache_menu_thread():
    """
    টপ সার্ভিস ও দেশের অপশন রেডি রাখা
    """
    global cached_services_kb, cached_countries_kb, smsify_dynamic_data
    while True:
        try:
            if smsify_dynamic_data:
                service_hits = [(sid, s_data, sum(r["hits"] for r in s_data["ranges"].values())) for sid, s_data in smsify_dynamic_data.items()]
                top_services = sorted(service_hits, key=lambda x: x[2], reverse=True)[:10]
                ...
                for sid, data, hits in top_services:
                    ...
                    row.append({
                        "text": safe_sid, 
                        "callback_data": f"srv_{safe_sid[:15]}", 
                        "style": styles[style_idx % 3], 
                        "icon_custom_emoji_id": data["emoji_id"]
                    })
                    style_idx += 1
                    if len(row) == 2:
                        new_srv_kb["inline_keyboard"].append(row)
                        row = []
                ...
                for sid, data, hits in top_services:
                    ...
                    for r, r_data in data["ranges"].items():
                        ...
                        if c_code not in country_map:
                            country_map[c_code] = {"name": c_name, "hits": 0}
                        country_map[c_code]["hits"] += r_data["hits"]

                    top_countries = sorted(country_map.items(), key=lambda x: x[1]["hits"], reverse=True)
                    cnt_kb = {"inline_keyboard": []}
                    styles_c = ["primary", "success", "danger"]; style_idx_c = 0
                    rate = get_otp_reward(sid)
                    ...
                    for c_code, c_data in top_countries:
                        ...
                        if emoji_char in COUNTRY_EMOJI_IDS:
                            btn["icon_custom_emoji_id"] = COUNTRY_EMOJI_IDS[emoji_char]
...
def init_firebase_from_file():
    global current_db_mode, db_firebase, bot_settings, force_join_status, force_join_channels, otp_forward_groups, otp_button_link, smsify_keys, smsify_auto_mode
    if os.path.exists("temp_firebase.json"):
        try:
            ...
            if doc.exists:
                ...
                if "smsify_keys" in data: smsify_keys = data["smsify_keys"]
                if "smsify_auto_mode" in data: smsify_auto_mode = data["smsify_auto_mode"]
        except Exception: pass
...
def update_user_stats(user_id, balance_add, invite_add):
    ...
    if user_id in user_cache:
        user_cache[user_id]["balance"] = round(user_cache[user_id]["balance"] + bal_add, 2)
...
def get_main_keyboard(user_id):
    keyboard_layout = [
        ...
        [
            {"text": "SUPPORT", "icon_custom_emoji_id": "5334880948259427772", "style": "primary"},
            {"text": "TELEGRAM GROUP", "icon_custom_emoji_id": "5429208347159077664", "style": "success"}
        ]
    ]
    if user_id == ADMIN_ID:
...
def get_force_join_alert_keyboard():
    ...
    for ch in force_join_channels:
        ...
        kb["inline_keyboard"].append([{"text": f"JOIN {name}", "url": url, "style": "primary"}])
    kb["inline_keyboard"].append([{"text": "✅ I HAVE JOINED", "callback_data": "check_fj_joined", "style": "success"}])
    return kb

def get_smsify_keys_keyboard():
    kb = {"inline_keyboard": []}
    status_text = "AUTO ALLOC: ON" if smsify_auto_mode else "AUTO ALLOC: OFF"
    status_icon = "5352694861990501856" if smsify_auto_mode else "5420130255174145507"
    kb["inline_keyboard"].append([{"text": status_text, "callback_data": "toggle_vk", "icon_custom_emoji_id": status_icon, "style": "primary" if smsify_auto_mode else "danger"}])
    
    for idx, key in enumerate(smsify_keys):
    ...
    kb["inline_keyboard"].append([{"text": "Add SMSIFY Key", "callback_data": "add_vk", "icon_custom_emoji_id": "5429501315468270290", "style": "success"}])
    kb["inline_keyboard"].append([{"text": "Back", "callback_data": "back_to_admin", "icon_custom_emoji_id": "5438541186539232243", "style": "primary"}])
    return kb
...
def get_admin_inline_keyboard():
    ...
    return {
        "inline_keyboard": [
            [{"text": "SMSIFYHUB SETTINGS", "callback_data": "manage_voltx", "icon_custom_emoji_id": "5240122519326860770", "style": "danger"}],
            [{"text": "BROADCAST", "callback_data": "broadcast_menu", "icon_custom_emoji_id": "5296633779157243809", "style": "primary"},
             {"text": "FORCE JOIN", "callback_data": "manage_fj", "icon_custom_emoji_id": "5429353834881261942", "style": "success"}],
            [{"text": "OTP GROUP", "callback_data": "manage_otp_group", "icon_custom_emoji_id": "5429208347159077664", "style": "danger"},
             {"text": "USER CONTROL", "callback_data": "user_control", "icon_custom_emoji_id": "5429274992166609833", "style": "primary"}],
            [{"text": "BABY CONTROL", "callback_data": "dxa_control", "icon_custom_emoji_id": "5240451569656308978", "style": "danger"}],
            [{"text": "UPLOAD FIREBASE", "callback_data": "upload_firebase", "icon_custom_emoji_id": "5429362935916962442", "style": "primary"}],
            [{"text": db_status, "callback_data": "db_status", "icon_custom_emoji_id": "5429591316507963112", "style": "success"}],
            [{"text": "CLOSE", "callback_data": "close_panel", "icon_custom_emoji_id": "5438541186539232243", "style": "danger"}]
        ]
    }

def baby_control_keyboard():
    grp = bot_settings.get("w_group", "")
    grp_status = grp if grp else "NOT SET"
    sup = bot_settings.get("support_link", "NOT SET")
    tgg = bot_settings.get("tg_group_link", "NOT SET")
    if len(sup) > 18: sup = sup[:15] + "..."
    if len(tgg) > 18: tgg = tgg[:15] + "..."
    ...
    return {"inline_keyboard": [
        ...
        [{"text": f"SUPPORT LINK: {sup}", "icon_custom_emoji_id": "5334880948259427772", "callback_data": "dxa_sup_link", "style": "primary"},
         {"text": f"GROUP LINK: {tgg}", "icon_custom_emoji_id": "5429208347159077664", "callback_data": "dxa_tg_group", "style": "success"}],
...
def baby_manage_w_methods_keyboard():
    kb = {"inline_keyboard": []}
...
def get_leaderboard_text():
    ...
    if cached_leaderboard_text: return cached_leaderboard_text
...
def get_live_traffic_content():
    ...
    for h in hits_to_show:
        raw_sid = h.get("service") or h.get("sid") or "Unknown"
        raw_msg = h.get("message") or h.get("sms") or ""
...
# ==========================================
# Dynamic Service & Country Content
# ==========================================
def get_services_content():
    global cached_services_kb, smsify_dynamic_data
    if cached_services_kb and cached_services_kb.get("inline_keyboard"):
        return "📱 <b>Select Top Live Service:</b>", cached_services_kb
        
    # লাইভ 2oo9 Access API থেকে সার্ভিস লোড করা
    live_services = fetch_live_access()
    
    # যদি কোনো কারণে লাইভ এপিআই খালি বা ডাউন থাকে, স্বয়ংক্রিয় ব্যাকআপ হিসেবে প্রিমিয়াম সার্ভিসগুলো লোড হবে
    if not live_services:
        live_services = [
            {"sid": "Telegram", "ranges": ["22501XXX", "8801XXX", "1XXX", "44XXX"]},
            {"sid": "WhatsApp", "ranges": ["1XXX", "44XXX", "62XXX", "91XXX"]},
            {"sid": "Facebook", "ranges": ["1XXX", "48XXX", "380XXX", "8801XXX"]},
            {"sid": "Instagram", "ranges": ["1XXX", "44XXX", "7XXX"]},
            {"sid": "TikTok", "ranges": ["1XXX", "44XXX", "62XXX"]},
            {"sid": "Google", "ranges": ["1XXX", "44XXX", "48XXX"]}
        ]

    new_srv_kb = {"inline_keyboard": []}
    row = []; styles = ["primary", "success", "danger"]; style_idx = 0
    for srv in live_services[:10]:
        sid = srv.get("sid", "Other")
        safe_sid = html.escape(str(sid))
        app_info = PREMIUM_APPS.get(sid, PREMIUM_APPS.get(sid.capitalize(), PREMIUM_APPS["Other"]))
        row.append({"text": safe_sid, "callback_data": f"srv_{safe_sid[:15]}", "style": styles[style_idx % 3], "icon_custom_emoji_id": app_info["id"]})
        style_idx += 1
        if len(row) == 2:
            new_srv_kb["inline_keyboard"].append(row)
            row = []
...
def get_countries_content(sid_short):
    global cached_countries_kb, smsify_dynamic_data
    if sid_short in cached_countries_kb:
        return "🌍 <b>Select Country:</b>", cached_countries_kb[sid_short], None

    ranges = []
    for sid, data in smsify_dynamic_data.items():
        if sid.lower().startswith(sid_short.lower()):
            ranges = list(data["ranges"].keys())
            break
            
    if not ranges:
        live_srvs = fetch_live_access()
        for s in live_srvs:
            if s.get("sid", "").lower().startswith(sid_short.lower()):
                ranges = s.get("ranges", [])
                break

    # ফলব্যাক ডিফল্ট রেঞ্জ লিস্ট
    if not ranges:
        ranges = ["1XXX", "44XXX", "48XXX", "380XXX", "7XXX", "91XXX", "62XXX", "84XXX"]
    ...
    cnt_kb = {"inline_keyboard": []}
    styles_c = ["primary", "success", "danger"]; style_idx_c = 0
    rate = get_otp_reward(sid_short)

    for c_code, c_name_full in country_map.items():
        parts = c_name_full.split(" ", 1)
        emoji_char = parts[0]
        clean_name = parts[1] if len(parts) > 1 else c_name_full
        btn_text = f"Other (+{c_code}) - {rate} TK" if "Other" in clean_name else f"{clean_name} (+{c_code}) - {rate} TK"
        btn = {"text": btn_text, "callback_data": f"cc_{sid_short}_{c_code}", "style": styles_c[style_idx_c % 3]}
        if emoji_char in COUNTRY_EMOJI_IDS:
            btn["icon_custom_emoji_id"] = COUNTRY_EMOJI_IDS[emoji_char]
        cnt_kb["inline_keyboard"].append([btn])
        style_idx_c += 1
    ...
    cnt_kb["inline_keyboard"].append([{"text": "BACK", "callback_data": "back_to_services", "icon_custom_emoji_id": "5438541186539232243", "style": "danger"}])
    cached_countries_kb[sid_short] = cnt_kb
    return "🌍 <b>Select Country:</b>", cnt_kb, None

def get_number_allocation_content(sid_short, c_code, user_id=None):
    global smsify_auto_mode, smsify_dynamic_data, user_active_sessions, active_sessions, bot_settings, otp_button_link
    target_ranges = []
    full_sid = sid_short
    ...
    if not best_ranges: best_ranges = [c_code]
    ...
    for _ in range(req_count):
        ...
        clean_selected = str(selected_rid).replace("X", "").replace("x", "")
        target_wildcard = clean_selected + "XXX"
        
        # SMSIFYHUB API কল
        num_data = smsify_get_number(target_wildcard, service=full_sid.lower())
        if num_data:
            full_number = str(num_data.get("number", num_data.get("phone", "")))
            session_id = num_data.get("session_id", num_data.get("id"))
            country = num_data.get("country", "Global")
            operator = num_data.get("operator", "Auto")
...
def handle_message(message):
    global waiting_for_firebase, current_db_mode, db_firebase, user_states, smsify_keys, force_join_channels, otp_forward_groups, otp_button_link, bot_settings
    ...
    if "chat_shared" in message:
        ...
        if user_id == ADMIN_ID and user_id in user_states:
            ...
            if state == "waiting_otp_group":
                ...
                save_local_data()
                if target_msg_id: edit_message(chat_id, target_msg_id, "⚙️ <b>BABY CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=baby_control_keyboard())
                del user_states[user_id]
                return
    ...
    if not is_user_joined(user_id):
        send_message(chat_id, "⚠️ <b>Please join our channels to use the bot!</b>", reply_markup=get_force_join_alert_keyboard())
        return
    ...
    if user_id == ADMIN_ID and user_id in user_states:
        ...
        target_msg_id = state_data.get("msg_id") if isinstance(state_data, dict) else None
            
        if state == "waiting_vk_key":
            smsify_keys.append(text.strip())
            save_local_data()
            if target_msg_id: edit_message(chat_id, target_msg_id, "🔑 <b>SMSIFY KEY SYSTEM</b>\nManage API Keys below:", reply_markup=get_smsify_keys_keyboard())
            del user_states[user_id]
            ...
            if bal < amount:
                ...
                if target_msg_id: delete_message(chat_id, target_msg_id)
        ...
        elif state == "waiting_dxa_sup_link":
            if text.startswith("http") or text.startswith("@"):
                bot_settings["support_link"] = text.strip()
                save_local_data()
                if target_msg_id: edit_message(chat_id, target_msg_id, "⚙️ <b>BABY CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=baby_control_keyboard())
            else:
                if target_msg_id: edit_message(chat_id, target_msg_id, "❌ <b>Invalid link format. Must start with http or @.</b>", reply_markup=get_back_only_keyboard())
            del user_states[user_id]
            delete_message(chat_id, message_id)
            return

        elif state == "waiting_dxa_tg_group":
            if text.startswith("http") or text.startswith("@"):
                bot_settings["tg_group_link"] = text.strip()
                save_local_data()
                if target_msg_id: edit_message(chat_id, target_msg_id, "⚙️ <b>BABY CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=baby_control_keyboard())
            else:
                if target_msg_id: edit_message(chat_id, target_msg_id, "❌ <b>Invalid link format. Must start with http or @.</b>", reply_markup=get_back_only_keyboard())
    ...
    if text in ["LIVE NUMBER", "LIVE TRAFFIC", "INVITE FRIEND", "WALLET", "LEADERBOARD", "SUPPORT", "TELEGRAM GROUP", "OWNER PANEL"]:
        if user_id in user_states: del user_states[user_id]
    ...
    if text.startswith("/start"):
        ...
        sup_url = bot_settings.get("support_link", "")
        if sup_url.startswith("@"): sup_url = "https://t.me/" + sup_url.replace("@", "")
        if not sup_url: sup_url = f"https://t.me/{BOT_USERNAME}"
        
        msg = (
            "<blockquote><tg-emoji emoji-id=\"5334880948259427772\">🖥</tg-emoji> <b>CUSTOMER SUPPORT</b></blockquote>\n"
            "━━━━━━━━━━━━━━━━━\n"
            "If you face any issues, click the button below to contact our official support."
        )
        kb = {"inline_keyboard": [[{"text": "CONTACT SUPPORT", "url": sup_url, "icon_custom_emoji_id": "5429501315468270290", "style": "primary"}]]}
        send_message(chat_id, msg, reply_markup=kb)

    elif text == "TELEGRAM GROUP":
        grp_url = bot_settings.get("tg_group_link", "")
        if grp_url.startswith("@"): grp_url = "https://t.me/" + grp_url.replace("@", "")
        if not grp_url: grp_url = f"https://t.me/{BOT_USERNAME}"
        
        msg = (
            "<blockquote><tg-emoji emoji-id=\"5429208347159077664\">💬</tg-emoji> <b>OFFICIAL COMMUNITY</b></blockquote>\n"
            "━━━━━━━━━━━━━━━━━\n"
            "Join our Telegram Group to get live updates, tips, and user discussions!"
        )
        kb = {"inline_keyboard": [[{"text": "JOIN GROUP", "url": grp_url, "icon_custom_emoji_id": "5314366268898300483", "style": "success"}]]}
        send_message(chat_id, msg, reply_markup=kb)
...
def handle_callback(callback_query):
    global user_states, smsify_auto_mode, current_db_mode, force_join_status, force_join_channels, smsify_keys, otp_forward_groups, bot_settings
    ...
    if data.startswith("srv_"):
        ...
        if err:
            answer_callback_query(query_id, err, show_alert=True)
        else:
            edit_message(chat_id, message_id, msg, reply_markup=markup)
            answer_callback_query(query_id)
        return
    ...
    if data.startswith("admin_w_approve_") or data.startswith("admin_w_reject_"):
        ...
        masked_number = f"{acc_number[:3]}BABY{acc_number[-3:]}" if len(acc_number) >= 6 else acc_number
    ...
    if data == "manage_voltx":
        edit_message(chat_id, message_id, "🔑 <b>SMSIFY KEY SYSTEM</b>\nManage API Keys below:", reply_markup=get_smsify_keys_keyboard())
        answer_callback_query(query_id)
    elif data == "toggle_vk":
        smsify_auto_mode = not smsify_auto_mode
        save_local_data()
        edit_message(chat_id, message_id, "🔑 <b>SMSIFY KEY SYSTEM</b>\nManage API Keys below:", reply_markup=get_smsify_keys_keyboard())
        answer_callback_query(query_id, f"SMSIFY Auto: {'ON' if smsify_auto_mode else 'OFF'}")
    elif data == "add_vk":
        user_states[user_id] = {"state": "waiting_vk_key", "msg_id": message_id}
        edit_message(chat_id, message_id, "🔑 <b>Please reply to this message with the new SMSIFY API Key.</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
    elif data.startswith("del_vk_"):
        idx = int(data[7:])
        if idx < len(smsify_keys): 
            if len(smsify_keys) > 1:
                smsify_keys.pop(idx)
                save_local_data()
                edit_message(chat_id, message_id, "🔑 <b>SMSIFY KEY SYSTEM</b>\nManage API Keys below:", reply_markup=get_smsify_keys_keyboard())
    ...
    # BABY Control Panel
        ...
        edit_message(chat_id, message_id, "⚙️ <b>BABY CONTROL PANEL</b>\nManage core bot configurations:", reply_markup=baby_control_keyboard())
        answer_callback_query(query_id)
    elif data == "dxa_min_w":
        ...
        user_states[user_id] = {"state": "waiting_dxa_sup_link", "msg_id": message_id}
        edit_message(chat_id, message_id, "🖥 <b>Enter new Support Link/Username (e.g. https://t.me/support):</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
    elif data == "dxa_tg_group":
        user_states[user_id] = {"state": "waiting_dxa_tg_group", "msg_id": message_id}
        edit_message(chat_id, message_id, "💬 <b>Enter new Telegram Community Group Link (e.g. https://t.me/group):</b>", reply_markup=get_back_only_keyboard())
        answer_callback_query(query_id)
...
def main():
    ...
    # ব্যাকগ্রাউন্ড লিসেনার থ্রেড চালু
    threading.Thread(target=live_access_feed_listener, daemon=True).start()
    threading.Thread(target=otp_polling_listener, daemon=True).start()
    threading.Thread(target=auto_cache_menu_thread, daemon=True).start()
    
    print(f"Bot @{BOT_USERNAME} running with 2oo9 Live Services & SMSIFYHUB Engine...")
